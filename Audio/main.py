...
import gopro
import gps
from Classificazione_tcn import Classificazione_Audio
...
if __name__ == "__main__":
    ...
    if success:
        ...
        #analisi_pint thread
        classificatore = Classificazione_Audio(gps_path=Gps.get_folder(), tcn_path=output_folder)
        classificazione_thread = threading.Thread(target=classificatore.analyze)
        classificazione_thread.start()
        
        pint.download_and_process_after_recording()
        
        print("finito")
        classificazione_thread.join()
        pint.stop_queue()
        th.join()
